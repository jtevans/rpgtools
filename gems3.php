...
function build_item()
{
  ...
  for ($x = 0; $x < count($cats); ++$x)
  {
    if ($roll1 >= $cats[$x]->low && $roll1 <= $cats[$x]->high)
    {
      ...
      $item .= $value . " GP. <span class=\"pt8\">(" . $cats[$x]->name . ", DC: " . $cats[$x]->dc . ")";
      if ($cats[$x]->dc >= 15)
      {
        $item .= " (No value known if appraise fails)</span>\n";
      }
      else
      {
        $item .= " (Misappraised Value: " . intval($value * ((dice(2, 6) + 3) * .1)) . "GP)</span>\n";
      }
      return($item);
    }
  }
...
<p align="center">
<a href="gems3.php?amount=<?php print($amount); ?>">Generate More Gemstones</a>
</p>
